...
uniform sampler2D uHit,uPath,uProfile,uSpectrum;
uniform float uTime,uAzimuth,uExposure,uHighlight;
uniform int uMode;
...
vec3 sky(float mu,float phi){
 vec2 p=vec2((mod(phi+PI,2.*PI))/(2.*PI)*1200.,acos(clamp(mu,-1.,1.))/PI*600.);vec3 rgb=vec3(0);
 for(int dy=-1;dy<=1;++dy)for(int dx=-1;dx<=1;++dx){
  ivec2 cell=ivec2(floor(p))+ivec2(dx,dy);cell.x=(cell.x%1200+1200)%1200;uint h=hashGrid(cell,1977u);
  if(h%1000u>=13u)continue;
  vec2 offset=vec2(float((h>>8u)&255u),float((h>>16u)&255u))/255.;
  vec2 d=p-(floor(p)+vec2(dx,dy)+offset);d.x*=sin(clamp(p.y/600.*PI,.1,PI-.1));
  float amp=(.004+.045*pow(float((h>>18u)&1023u)/1023.,5.))*exp(-dot(d,d)/(2.*.075*.075));
...
void main(){
 ...
 }else if(path.x>1.5&&uMode<1)rgb=sky(path.z,path.w+uAzimuth);
...
precision highp float;
in vec2 uv;out vec4 color;
uniform sampler2D uColor;uniform vec2 uSize;uniform float uExposure,uBloom;
vec3 exposed(vec2 p){vec3 c=texture(uColor,p).rgb*uExposure;return c/(1.+max(c.r,max(c.g,c.b)));}
void main(){
 vec3 c=exposed(uv),glow=vec3(0);vec2 d=1./uSize;
 for(int j=0;j<8;++j){float ang=float(j)*.785398163;vec2 v=vec2(cos(ang),sin(ang));
