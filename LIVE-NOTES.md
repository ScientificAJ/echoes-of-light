...
The disk retains its original multiscale procedural texture and fine filament structure. Spatial filtering of finite source profiles is applied to the background stars; disk detail retains the original single-ray sampling limits. The tone-map shoulder is evaluated once in the shading pass before display interpolation, instead of repeating it for every bloom tap. Interpolation and nonlinear tone mapping do not commute exactly, so this is a small display-response change. Bloom is bypassed entirely when its strength is zero.
